...
def Trainer(opt):
    ...
    jpg_list_train, jpg_list_test, png_list_train, png_list_test = PathList(path, train_ratio = 1)
    ...
    for epoch in range(opt.epochs):
        ...
        if (epoch+1) % opt.checkpoint_interval == 0:
            torch.save(FirstP, 'FirstPNet_ACDC_2017_epoch%d.pth' % (epoch+1))
...
def Testing():
    ...
    FirstP = torch.load('/home/xjj/Desktop/segmentation/FirstPNet_ACDC_2017_epoch60.pth').cuda()
    ...
    for batch_idx, (inputs, inputs_gray, inputs_sobel, inputs_laplacian, targets, targets_mask, targets_pixel) in enumerate(TestLoader):
        ...
        pixel = pixel_logits.view(46, 46)
        pixel = pixel.cpu().detach().numpy()
